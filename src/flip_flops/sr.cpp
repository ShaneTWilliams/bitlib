...
bl::SrFlipFlop::SrFlipFlop(std::string name)
    : Component(name),
      ...
      clk_buf(BufferGate(name + "::CLK_BUF")) {
    bl::connect(this->s_and.output, this->sr_latch.s);
    bl::connect(this->r_and.output, this->sr_latch.r);

    bl::connect(this->clk_buf.output, this->s_and.inputs[0]);
    bl::connect(this->clk_buf.output, this->r_and.inputs[0]);

    this->s = s_and.inputs[1];
    this->r = r_and.inputs[1];
    this->q = sr_latch.q;
    this->qn = sr_latch.qn;
    this->clk = clk_buf.input;
